...
try:
    ...
    from models_minigrid import DiscreteActor # Key change: DiscreteActor
...
def compute_lambda_values(next_values, rewards, dones, discount, lambda_):
    ...
    # We need the value at H+1 (bootstrap) which is passed in as next_values[:, -1]?
    # Usually next_values matches shape of rewards (values at t+1).
    # The last next_value is the bootstrap.
    ...
    for t in reversed(range(horizon)):
        ...
        td = rewards[:, t] + (1.0 - dones[:, t]) * discount * next_values[:, t]
        
        # Lambda return: (1-lambda)*TD + lambda*G_{t+1}
        # Actually standard formula: G_t = r + gamma * ((1-lambda)*v(s') + lambda*G_{t+1})
        # If lambda=1, G_t = r + gamma * G_{t+1} (MC)
        # If lambda=0, G_t = r + gamma * v(s') (TD)
        
        # We blend the BOOTSTRAP:
        # target = r + gamma * ( (1-lambda)*V(s') + lambda*v_lambda_next )
...
def update_ebm_online(ebm, buffer, optimizer, num_steps=50, batch_size=32):
    ...
    for _ in range(num_steps):
        ...
        # Generate Negatives? 
        # For online speed, maybe just use random buffer samples (Contrastive)
        # Or mini-Langevin.
        # Let's use simple random buffer negatives (InfoNCE default) 
        # as Langevin is expensive inside policy loop.
...
class TrajectoryBuffer:
    ...
    def sample_imagination_states(self, batch_size):
        ...
        if len(self.recent_trajectories) > 0:
            # Flatten recent trajectories for sampling
            # (Or just sample randomly from them)
            # Sampling logic: Pick a trajectory, then pick a state
            recent_states = []
...
def evaluate_policy(env_adapter, actor, num_episodes=5):
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            with torch.no_grad():
                # For eval, we can use soft sample ARGMAX
    ...
    while total_steps < CONFIG["TOTAL_STEPS"]:
        ...
        if state_buffer.size >= CONFIG["BATCH_SIZE"] and total_steps % 10 == 0:
            ...
            for t in range(CONFIG["HORIZON"]):
                ...
                # Add entropy bonus to reward for actor objective?
                # Or keep separate. Standard soft-actor-critic adds to reward.
                # r_augmented = r_pred + alpha * entropy
                # Let's add it to rewards_list for "Actor Objective" calculation, 
                # but NOT for Critic Training (Critic should learn True Return).
                # Actually, in MaxEnt RL, Critic learns soft value.
                # Let's keep it simple: Add entropy term to Actor Loss directly.
            ...
            with torch.no_grad():
                # Target Policy Action for bootstrapping? V(s) doesn't need action estimate.
                # V(s') is just V_net(s').
            ...
            # Gradient flows through: V(ns) -> ns -> (Model) -> a -> Actor
            
            # We treat the whole trajectory as a single graph.
            # We want to maximize the Value of the states we visited.
            # Actor Loss = - Mean( r_t + gamma * V_target( s_{t+1} ) )
            # Not V_target, but V_current (differentiable) to get gradients for 's_{t+1}'
            
            # Reconstruct flow for gradients:
            # We have next_states_seq (with grad) and rewards_seq (with grad)
            
            # Compute Values of next states using CRITIC (differentiable w.r.t input)
            # Note: We keep Critic FIXED here to avoid "pulling up the critic"? 
            # Usually in DPG, we freeze Critic parameters but backprop through inputs (state/action).
            # Here V(s) parameters are fixed, but s depends on Actor.
...
def train():
    print(f"Policy Training on {CONFIG['ENV_NAME']}")
    device = CONFIG["DEVICE"]
